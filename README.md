...
- 영상 생성: 이미지 기반 영상 생성, TTS 더빙, BGM 믹스, 자막 포함 샷 합성
...
이미지 생성, 영상 생성, TTS 생성, BGM 생성은 기본적으로 외부 생성 모델을 호출하므로 해당 provider의 API 키가 필요합니다. 다만 비용이 커질 수 있는 미디어 작업은 아래 하이브리드 생성 모드로 프롬프트만 추출한 뒤 사용자가 구독형 서비스에서 반자동으로 생성하고 결과 파일 또는 URL을 등록하는 흐름을 사용할 수 있습니다.
...
- `API 자동 생성`: 설정에 저장한 provider API 키를 사용해 이미지, 영상, TTS, BGM을 자동 생성하는 방향입니다.
- `수동/구독형 생성`: 앱이 프롬프트를 만들고, 사용자는 외부 구독형 LLM/이미지/영상/TTS/BGM 서비스에서 생성한 결과 파일 또는 URL을 등록합니다.
...
이미지와 영상은 Google Flow 같은 웹 구독형 서비스에서 생성한 파일을 등록할 수 있고, BGM은 Suno 같은 서비스에 붙여 넣기 쉬운 프롬프트를 샷별 또는 일괄로 복사할 수 있습니다. 설정에서 텔레그램/디스코드 원격 승인 채널 정보를 저장해 두는 UI도 준비되어 있으며, 현재는 브라우저 로컬 설정으로 보관되는 승인 흐름 준비 단계입니다.

영상 합성 단계에서는 더빙 합성/원본 음성 유지와 별도로 `BGM 없음` 또는 `BGM 믹스`를 선택할 수 있습니다. `BGM 믹스`를 선택하면 스토리보드에 등록된 BGM 파일을 FFmpeg 합성 단계에서 더빙 또는 원본 영상 음성과 함께 섞습니다.

### 브라우저 자동화 보조 확장 프로그램

`browser-extension/` 폴더에는 Chrome 확장 프로그램 형태의 자동화 보조 도구가 들어 있습니다. 이 확장 프로그램을 설치하면 회차 작업대의 `프롬프트/등록` 창에서 `Flow로 보내기` 또는 `Suno로 보내기` 버튼을 눌러 대상 웹서비스 탭을 열고 프롬프트 입력창에 내용을 자동으로 넣을 수 있습니다. 입력 후에는 서비스 화면 오른쪽 아래 자동화 패널에서 생성 버튼 후보와 다운로드 버튼 후보를 찾고, 사용자가 승인하면 클릭합니다.

설치 방법:

1. Chrome에서 `chrome://extensions`를 엽니다.
2. `개발자 모드`를 켭니다.
3. `압축해제된 확장 프로그램을 로드`를 누르고 `browser-extension` 폴더를 선택합니다.
4. 앱을 `http://localhost:3013`에서 열고 수동 생성 모달의 자동화 버튼을 사용합니다.

현재 확장 프로그램은 프롬프트 입력과 버튼 후보 탐지까지 자동화하고, 실제 생성/다운로드 클릭은 사용자가 자동화 패널에서 승인해야 실행합니다. 로그인, 정책 확인, 유료 크레딧 사용 여부, 생성 옵션은 서비스 화면에서 직접 확인하는 흐름으로 남겨 둡니다.
...
- 회차 작업대의 캐릭터, 장면, 샷 프레임, 더빙, 영상, BGM 버튼을 토글 상태에 따라 `자동 생성` 또는 `프롬프트 복사/결과 등록` 흐름으로 전환했습니다.
- 수동 등록 모달에서 결과 파일 업로드, 외부 URL 다운로드, 기존 `static/...` 경로 연결을 처리합니다.
- 회차 생성 시 이미지/더빙/영상/BGM의 기본 생성 방식을 각각 고정하고, 회차 내부에서도 서비스별로 다시 전환할 수 있습니다.
- Chrome 확장 프로그램으로 Flow/Suno 웹 화면에 프롬프트를 전달하는 보조 자동화 경로를 추가했습니다.

다음 작업:

- 실제 생성 파일을 사용해 이미지, TTS, 영상, BGM의 end-to-end 수동 등록 흐름을 프로젝트 데이터에서 검증합니다.
- 필요하면 합성 완료 영상도 외부에서 만든 파일로 직접 연결하는 보조 등록 버튼을 추가합니다.
- 텔레그램/디스코드로 승인 요청을 실제 발송하고 승인 결과를 작업대 상태에 반영하는 backend 연동을 추가합니다.
